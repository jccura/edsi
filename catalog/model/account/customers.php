...
class ModelAccountCustomers extends Model {	
	...
	public function getTotalWholesalerList($user_id, $data = array()) {
		$sql  = " SELECT count(1) as total from oc_unilevel a
					join oc_user b on(a.user_id = b.user_id)
					where b.user_group_id = 56 and a.sponsor_user_id = ".$user_id;
		$query = $this->db->query($sql);
		return $query->row['total'];	
	}
	
	public function getTotalResellerList($user_id, $data = array()) {
		$sql  = " SELECT count(1) as total from oc_unilevel a
					join oc_user b on(a.user_id = b.user_id)
					where b.user_group_id = 46 and a.sponsor_user_id = ".$user_id;
		$query = $this->db->query($sql);
		return $query->row['total'];	
	}
	
	public function getTotalCitydistributorList($user_id, $data = array()) {
		$sql  = " SELECT count(1) as total from oc_unilevel a
					join oc_user b on(a.user_id = b.user_id)
					where b.user_group_id = 57 and a.sponsor_user_id = ".$user_id;
		$query = $this->db->query($sql);
		return $query->row['total'];	
	}
	...
	function getGeneologyList($user_id, $data = array()) {	
		...
		$sql  = "select a.user_id dl_user_id,sum(d.sales_month) + sum(d.prev_month) + sum(d.sales_second_month) sales,
				  ...
				  left join oc_sales_delivered d on(a.user_id = d.user_id)
				 where a.sponsor_user_id = ".$user_id."
			      group by a.user_id
		              order by a.level  
	...
	function getGroupsales($user_id, $data = array()) {	
		if($data['start'] < 0) {
			$data['start'] = 0;
		}
		...
		$sql  = " select a.user_id dl_user_id, 
					     b.username dl_username, concat(b.firstname, ' ', b.lastname) dl_desc, 
						 c.username sp_username, concat(c.firstname, ' ', c.lastname) sp_desc,
						 a.level ,d.amount sales,
						 case when b.activation_flag = 1 then 'Active' else 'In-Active' end act_flag
				  from oc_unilevel a
				  left join oc_user b on(a.user_id = b.user_id)
				  left join oc_user c on(c.user_id = b.refer_by_id)
				  left join oc_order d on(d.user_id = a.user_id)
				 where a.sponsor_user_id = ".$user_id." and d.paid_date >= '2020-10-01 00:00:00' 
						  and d.paid_date <= '2020-12-31 23:59:59'
				 order by a.level 
				 LIMIT " . (int)$data['start'] . "," . (int)$data['limit'];
	...
	public function getNetworkExport($data) {
		
		$sql  = " select a.user_id dl_user_id, 
						 c.username sp_username, b.username dl_username,
						 concat(b.firstname, ' ', b.lastname) dl_desc,
						 a.level, b.date_added,
						 case when b.activation_flag = 1 then 'Active' else 'In-Active' end act_flag
				  from oc_unilevel a
				  left join oc_user b on(a.user_id = b.user_id)
				  left join oc_user c on(c.user_id = b.refer_by_id)
				 where a.sponsor_user_id = ".$this->user->getId()." and a.level <= 3";	
		//paymentoption
		
		//withincity
		
		$sql .= " ORDER BY a.level ";
	...
	function getWholesalerList($user_id, $data = array()) {	
		if($data['start'] < 0) {
			$data['start'] = 0;
		}
		...
		if(isset($data['search_user'])) {
			if(!empty($data['search_user'])){
				$sql .= " and lower(b.username) = '".strtolower($data['search_user'])."' ";
			}
		}	
	...
	function getResellerList($user_id, $data = array()) {	
		if($data['start'] < 0) {
			$data['start'] = 0;
		}
		...
		$sql  = " select a.user_id dl_user_id, 
					...
					left join oc_user b on(a.user_id = b.user_id)
					left join oc_user c on(c.user_id = b.refer_by_id)
					where a.sponsor_user_id = ".$user_id." and a.level <= 3 and b.user_group_id = 46 ";
	...
	public function getTotalgroupsales() {
		$sql = "select SUM(a.amount) sales
				  from oc_order a
				  join oc_unilevel b on (a.user_id = b.user_id)
				 where b.sponsor_user_id = ".$this->user->getId()."
				  and b.level <=6 and a.paid_date >= '2020-10-01 00:00:00' 

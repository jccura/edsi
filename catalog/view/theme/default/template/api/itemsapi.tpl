	...
	<div class="header-body">
	  <div class="row align-items-center py-4">
		<div class="col-lg-6 col-7">
			<i class="ni ni-compass-04 ni-2x text-white"></i><h6 class="h1 text-white d-inline-block mb-0">&nbsp; Sync Items</h6>
		</div>
	  </div>
	</div>
  </div>
									...
									<table class="table table-hover table-bordered">
									  ...
									  <tbody>
										<?php if (isset($item_info)) { ?>
											<?php foreach ($item_info as $ito) { ?>
												<tr>	
												<td><?php echo $ito['item_id'] ?></td>
												<td><?php echo $ito['category'] ?></td>
												<td ><?php echo $ito['item_code'] ?></td>
												<td ><?php echo $ito['item_name'] ?></td>
												<td ><?php echo number_format($ito['price'],2) ?></td>
												<td ><?php echo $ito['date_added'] ?></td>
													</tr>
												</td>		
											<?php } ?>
										<?php } ?>
...
function sync(){
	// $('#msg').html("Sync Items to Manong Express?");
	// $(function() {
		// $( "#dialog-message" ).dialog({
		  // modal: true,
		  // width: 600,
	...
	$('#msg_proceed').html("Sync Items to Manong Express?");
	$('#div_buttons').html("<button type=\"button\" onclick=\"performTask('syncitems');\" class=\"btn btn-danger\">Yes, Sync it.</button>");		

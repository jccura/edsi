	...
	<div class="header-body">
	  <div class="row align-items-center py-4">
		<div class="col-lg-6 col-7">
			<i class="ni ni-compass-04 ni-2x text-white"></i><h6 class="h1 text-white d-inline-block mb-0">&nbsp; Sync Category</h6>
		</div>
	  </div>
	</div>
  </div>
...
function sync(){
	// $('#msg').html("Sync Categories to Manong Express?");
	// $(function() {
		// $( "#dialog-message" ).dialog({
		  // modal: true,
		  // width: 600,
		  // buttons: {
			...
			// },
			// Proceed: function() {
				// $('#task').val('synccategory');
				// $('#form').attr('action', 'categoryapi'); 
				// $('#form').submit();
				// $( this ).dialog( "close" );
	...
	$('#msg_proceed').html("Sync Categories to Manong Express?");
	$('#div_buttons').html("<button type=\"button\" onclick=\"performTask('synccategory');\" class=\"btn btn-danger\">Yes, Sync it.</button>");		

		...
		<div class="header-body">
			<div class="row align-items-center py-4">
				<div class="col-lg-6 col-7">
					<i class="ni ni-single-02 ni-2x text-white"></i><h6 class="h1 text-white d-inline-block mb-0">&nbsp;&nbsp;Manage Operators</h6>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-12">
				<form action="" method="post" enctype="multipart/form-data" id="form">
						...
						<div id="admin-modal" class="modal" role="dialog">
							<div class="modal-dialog modal-lg">
								<div class="modal-content">
									<div class="modal-header bg-turqouise">
										<h5 class="modal-title text-white" id="exampleModalLabel"><i class="fa fa-user-plus"></i> Add Operator</h5>
                      ...
                      <button type="button" class="btn bg-turqouise text-white" data-dismiss="modal">Cancel</button>
						...
						<div class="row">
							<div class="col-md-6">
								 <div class="input-group input-group-alternative">
									...
									<button class="btn bg-turqouise text-white" type="button" onclick="javascript: processTask('search')"><i class="fa fa-search"></i> Search</button>
								</div>
							</div>
							<div class="col-md-6 text-right">
							<div><button class="btn bg-turqouise text-white pull-right" type="button" data-toggle="modal" data-target="#admin-modal" onclick="javascript: addNewOperatorShow()"><i class="fa fa-plus"></i> Add Operator</button></div>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="row">			
			<div class="col">
				<div class="card">
					<div class="card-header bg-transparent border-0">
					  <h3 class="mb-0">List of Operators</h3>
					</div>
					<div class="col-md-12">
						<div class="row">
                                 ...
                                 <tbody>
                                    <?php if (isset($operators)) { ?>								
                                       ...
                                       <tr>
										 ...
										 <td>
                                            <button class="btn btn-primary btn-sm" type="button" data-toggle="modal" data-target="#admin-modal" onclick="javascript: getOperatorDetails(<?php echo $wh['user_id'] ?>)"><i class="fa fa-pencil"></i> Edit</button>
                                            <?php if ($wh['status'] == 'Enabled') { ?>
												<button class="btn btn-danger btn-sm" type="button" onclick="javascript: disableOperator(<?php echo $wh['user_id'] ?>)"><i class="fa fa-remove"></i> Disable</button>
											<?php } else { ?>
												<button class="btn btn-success btn-sm" type="button" onclick="javascript: enableOperator(<?php echo $wh['user_id'] ?>)"><i class="fa fa-check"></i> Enable</button>
											<?php } ?>
										 </td>
                                       </tr>
                                       <?php } ?>								
...
function addNewOperator(user_group_id) {
  ...
  if(proceed == 0){
    ...
    $('#task').val('addNewOperator');
    $('form').attr('action', "manageoperator"); 
    $('form').submit();
...
function updateOperatorDetails() {
  var firstname = $("#firstname").val();
  var middlename = $("#middlename").val();
  var lastname = $("#lastname").val();
  var contactno = $("#contactno").val();
  ...
  if(checkout_provinces == 0 || checkout_provinces == "" || !checkout_provinces){
    msg += "Province is required.<br>";
    proceed = 0;
  }
  
  if(checkout_city == 0 || checkout_city == "" || !checkout_city ){
    msg += "City is required.<br>";
    proceed = 0;
  }
  
  if(checkout_barangay == 0 || checkout_barangay == "" || !checkout_barangay){
    msg += "Barangay is required.<br>";
    proceed = 0;
  }
  ...
  $('#task').val('updateOperatorDetails');
  $('form').attr('action', "manageoperator"); 
  $('form').submit();
  }
...
function disableOperator(user_id) {

  $("#user_id").val(user_id);
  $('#task').val('disableOperator');
  $('form').attr('action', "manageoperator"); 
  $('form').submit();

}

function enableOperator(user_id) {

  $("#user_id").val(user_id);
  $('#task').val('enableOperator');
  $('form').attr('action', "manageoperator"); 
  $('form').submit();

}

function getOperatorDetails(user_id) {
	
  $.ajax({
    url: 'getOperatorDetails/' + user_id,
    type: 'get',
    dataType: 'json',
    success: function(json) {             
      if (json['status'] == "success") {
        ...
        $("#modal-button-admin").html("<button type=\"button\" class=\"btn bg-turqouise\" onclick=\"updateOperatorDetails()\" ><i class=\"fa fa-floppy-o\"></i> Update Operator</button>");
...
function addNewOperatorShow(){
  ...
  $("#cust_address").val("");
  $("#modal-button-admin").html("<button type=\"button\" class=\"btn bg-turqouise text-white\" onclick=\"addNewOperator(39);\" ><i class=\"fa fa-plus\"></i> Add Operator</button>");
  // $("#admin-modal").modal('toggle');
}

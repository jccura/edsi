	...
	<div class="header-body">
	  <div class="row align-items-center py-4">
		<div class="col-lg-6 col-7">
			<i class="ni ni-like-2 ni-2x text-white"></i><h6 class="h1 text-white d-inline-block mb-0">&nbsp; Payment Confirmed</h6>
		</div>
	  </div>
	</div>
  </div>
								...
								<!--<div class="template-demo">-->
									<div class="row">
										...
										<?php if($this->user->getUserGroupId() == 43) { ?>
											<div class="col-md-3">
												<button class="btn btn-outline-warning btn-block" type="button" onclick="javascript: performTask('tagshipped');"><i class="fa fa-list"></i> Tag as Shipped</button>
												<br/>
											</div>
										<?php } ?>
									</div>
												...
												<?php if (isset($order_list)) { ?>
													<?php foreach ($order_list as $it) { ?>
														<tr>
															...
															<td>
																<input style="width:180px" class="form-control input" type="text" id="ship_tracking_input<?php echo $it['order_id'];?>" name="ship_tracking_input<?php echo $it['order_id'];?>" value="<?php if(isset($it['tracking'])) { echo $it['tracking']; } ?>">
															</td>
...
function performTask(task) {
	...
	$('#task').val(task);
	$('form').attr('action', 'paymentconfirmed'); 
	$('form').submit();

	...
	<div class="header-body">
	  <div class="row align-items-center py-4">
		<div class="col-lg-6 col-7">
			<i class="ni ni-cart ni-2x text-white"></i><h6 class="h1 text-white d-inline-block mb-0">&nbsp; Quick Delivery Pending</h6>
		</div>
	  </div>
	</div>
  </div>
...
function performTask(task) {
	...
	$('#task').val(task);
	$('form').attr('action', 'quickdelpending'); 
	$('form').submit();
}
...
function viewQD(qd_id) {
	...
	$('#task').val('view');
	$('#qd_id').val(qd_id);
	$('#visibility').val('to_hide');
	$('#referror').val('quickdelpending');

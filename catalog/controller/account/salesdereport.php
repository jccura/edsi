<?php   
class ControllerAccountSalesDeReport extends Controller {   
	public function index() {
		if (!$this->user->isLogged() or !($this->user->checkScreenAuth($this->user->getUserGroupId(), 'salesdereport'))) {
	  		$this->redirect("home");
    	}
		
		$this->load->model('account/salesdereport');
		...
		$this->data['salesencodedreport'] = $this->model_account_salesdereport->getSalesEncodedRep($data, "data");
		$this->data['summary'] = $this->model_account_salesdereport->getSalesEncodedRep($data, "summary");		
		$total = $this->data['total'] = $this->model_account_salesdereport->getSalesEncodedRep($data, "total");
		...
		$pagination->url = $this->url->link('account/salesdereport', $url . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();	
		
		$this->template = 'default/template/account/salesdereport.tpl';
		$this->children = array(
			'common/column_left',
			'common/footer',
			'common/header'	
	...
	public function exporttocsv(){
		...
		$export_data = $this->model_account_salesdereport->getSalesEncodedRep($data, "data");
		$summary = $this->model_account_ssalesdereport->getSalesEncodedRep($data, "summary");		
		$totals = array("",
					"",
					"",
					"",
